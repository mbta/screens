defmodule Screens.V2.RDSTest do
  ...
  defp expect_standard_route_patterns(stop_ids, patterns \\ nil) do
    ...
    expect(@route_pattern, :fetch, fn %{mode: :bus, stop_ids: ^stop_ids, typicality: 1} ->
      {:ok, patterns}
    end)
  end
  ...
  describe "get/1 countdowns" do
    test "creates destinations from upcoming predicted and scheduled departures" do
      ...
      expect(@departure, :fetch, fn
        %{direction_id: 0, mode: :bus, stop_ids: ["s0"]}, [{:now, ^now} | _] ->
          {
            :ok,
            expected_departures_one ++
              expected_departures_two ++
    ...
    test "filters out the drop off only departures at the current stop id and route patterns" do
      ...
      expect(@departure, :fetch, fn
        %{direction_id: 0, mode: :bus, stop_ids: ^stop_ids}, [{:now, ^now} | _] ->
          {
            :ok,
            expected_departures
          }
    ...
    test "filters out cancelled departures for stops with headways" do
      ...
      expect(@departure, :fetch, fn
        %{direction_id: 0, mode: :rl, stop_ids: ["s0"]}, [{:now, ^now} | _] ->
  ...
  describe "get/1 service ended" do
    ...
    test "creates service ended state when after last scheduled departure, same headsigns" do
      now = ~U[2024-10-11 10:50:00Z]
      stop_ids = ~w[s0 s1]
      ...
      first_schedule =
        %Schedule{
          ...
          route: %Route{id: "r1", line: %Line{id: "l1"}, type: :bus},
          stop: %Stop{id: "sA"},
          trip: %Trip{headsign: "h1", pattern_headsign: "h1"}
        }
    ...
    test "creates service ended state before last scheduled departure, same direction names" do
      ...
      expect(@departure, :fetch, fn
        %{direction_id: :both, mode: :bus, stop_ids: ^stop_ids}, [{:now, ^now} | _] ->
          {:ok, []}
      end)
      ...
      patterns = [
        ...
        %RoutePattern{
          id: "B",
          headsign: "hB",
          route: %Route{id: "r2", line: %Line{id: "l1"}, type: :bus},
          stops: [%Stop{id: "otherA"}, %Stop{id: "sB"}, %Stop{id: "otherY"}]
        },
        %RoutePattern{
          id: "C",
      ...
      expect(@last_trip, :last_trip_departure_times, fn {"sB", "l1", "hB"} ->
        [~U[2024-10-11 10:43:50Z]]
      end)

      expect(@last_trip, :last_trip_departure_times, fn {"sC", "l1", "hC"} ->
        [~U[2024-10-11 10:43:50Z]]
      end)
