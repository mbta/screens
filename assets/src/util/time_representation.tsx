...
export type TimeRepresentation = {type: "TEXT", text: string} | {type: "MINUTES", minutes: number} | {type: "TIMESTAMP", timestamp: string, ampm: string};

export const standardTimeRepresentation = (departureTimeString: string, currentTimeString: string, vehicleStatus: string): TimeRepresentation => {
...
export const einkTimeRepresentation = (departureTimeString: string, currentTimeString: string): TimeRepresentation => {
  const departureTime = moment(departureTimeString);
  const currentTime = moment(currentTimeString);
  const secondDifference = departureTime.diff(currentTime, "seconds");
  const minuteDifference = Math.round(secondDifference / 60);
