...
- Double check that any behavior specific to the DUP screen environment happens inside of an `isDup()` check. This includes:
  - `useApiPath` in [`use_api_response.ts`](/assets/src/hooks/use_api_response.ts) should return a full URL for the API path: the URL base should be "https://screens.mbta.com".
  - `imagePath` in [`util.ts`](/assets/src/util/utils.ts) should return relative paths (no leading `/`).

- Set the version string in [`version.tsx`](/assets/src/components/dup/version.tsx) to `current_year.current_month.current_day.1`.
...
- **Only if you are packaging for local testing:** To test against your local screens backend instead of production, then replace the definition of `OUTFRONT_BASE_URI` in [`use_api_response.ts`](/assets/src/hooks/use_api_response.ts) with `"http://localhost:4000"`.
...
Once you've created the client app packages, you'll need to send them to Outfront to test and deploy. Make sure to remove any changes that you made to [`outfront.tsx`](/assets/src/util/outfront.tsx) and [`use_api_response.ts`](/assets/src/hooks/use_api_response.ts) for local testing before generating packages for Outfront.
