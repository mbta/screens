...
- `cd` to priv/static and run the following:
  ...
  for ROTATION_INDEX in {0..2}; do
    ...
    cp -r css/packaged_dup_v2.css js/polyfills.js js/packaged_dup_v2.js js/packaged_dup_v2.js.map ../dup_preview.png ../dup-app.html . && \
    cp ../dup_template.json ./template.json && \
    sed -i "" "s/DUP APP ./DUP APP ${ROTATION_INDEX}/" template.json && \
    zip -r dup-app-${ROTATION_INDEX}.zip packaged_dup_v2.css polyfills.js packaged_dup_v2.js fonts images dup-app.html template.json dup_preview.png
  done
  ```
