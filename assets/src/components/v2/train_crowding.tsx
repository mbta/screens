...
const lookupCarComponent = (occupancyStatus: string, frontCarDirection: string | boolean) => {
  if (frontCarDirection) {
    if (occupancyStatus == "not_crowded") return frontCarDirection == "left" ? CarNotCrowdedLeft : CarNotCrowdedRight
    else if (occupancyStatus == "some_crowding") return frontCarDirection == "left" ? CarSomeCrowdingLeft : CarSomeCrowdingRight
    else if (occupancyStatus == "crowded") return frontCarDirection == "left" ? CarCrowdedLeft : CarCrowdedRight
    else if (occupancyStatus == "closed") return frontCarDirection == "left" ? CarClosedLeft : CarClosedRight
...
const TrainCrowding: React.ComponentType<Props> = ({
  ...
  const trainSequence = front_car_direction == "left" ? trains : ([] as ReactElement[]).concat(trains).reverse()
  ...
  const arrowSlot = [1, 9, 17].includes(platform_position) ?
        platform_position + 1
        : platform_position == 25 ? platform_position - 1 : platform_position
  ...
  return <div className="crowding-widget">
    ...
    <div className="crowding-widget__body">
      <div className="crowding-widget__train-row">
        {trainSequence}
      </div>
    ...
    <div className="crowding-widget__footer">
      ...
      <div className="crowding-widget__footer__segment">
        <div className="crowding-widget__footer__key-row">
          <KeyNotCrowded width="137" height="100" className="key-icon" /> Not crowded
        </div>
        <div className="crowding-widget__footer__key-row">
          <KeySomeCrowding width="137" height="100" className="key-icon" /> Some crowding
        </div>
      </div>
      <div className="crowding-widget__footer__segment">
        <div className="crowding-widget__footer__key-row">
        ...
        <div className="crowding-widget__footer__key-row">
          {
            crowding.includes("closed") ?
                <><KeyClosed width="137" height="100" className="key-icon" /> Closed</>
                : crowding.includes("no_data") && <><KeyNoData width="137" height="100" className="key-icon" /> No Data</>
          }
        </div>
      </div>
    </div>
