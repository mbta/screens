...
const InlineAlertBadge = ({alert}: InlineAlertBadgeProps): JSX.Element => {
  ...
  const withClassModifier = (className: string) => `${className} ${className}--${alert}`;

  return (
    <div className={withClassModifier("inline-alert-badge")}>
      <div className={withClassModifier("inline-alert-badge__icon-container")}>
        <img className={withClassModifier("inline-alert-badge__icon-image")} src={svgPath} />
      </div>
      <div className={withClassModifier("inline-alert-badge__text")}>{text}</div>
    </div>
  )
};
