defmodule Screens.V2.CandidateGenerator.Widgets.RealtimeDepartures do
  ...
  defp generate_instances(
    ...
    sections_data =
      departures
      |> @rds.get(now)
      |> RdsDepartures.create_departure_sections(departures, post_process_rows_fn)
  ...
  defp handle_unsupported_sections(
    ...
    route =
      case section_to_change do
        %HeadwaySection{route: route} -> route
        %NoDataSection{route: route} -> route
        %OvernightSection{routes: routes} -> Enum.at(routes, 0)
        %NoServiceSection{routes: routes} -> Enum.at(routes, 0)
      end

    text =
      case section_to_change do
        %HeadwaySection{headsign: headsign} ->
          %FreeTextLine{
