defmodule Screens.ScreensByAlert.SelfRefreshRunner do
  ...
  def handle_info(:check, refreshing_ids) when refreshing_ids == @empty_set do
    ...
    # To make efficient use of the batch size, only consider screen IDs owned by this node in the
    # partitioned screen data cache.
  ...
  defp should_refresh_on_this_node?(id) do
    case ScreenData.Cache.Store.find_node(id) do
      ...
      {:ok, _other} -> false
      {:error, _error} -> raise "cannot determine whether this node should self-refresh #{id}"
    end
  end
